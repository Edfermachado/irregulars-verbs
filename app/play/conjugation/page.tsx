...
export default function ConjugationPractice() {
  const { score, currentConjugation, options, feedback, checkAnswer, setGameMode } = useStore();
  ...
  if (!mounted || !currentConjugation) return null;
      ...
      <div className="flex-1 flex flex-col justify-center items-center w-full max-w-md px-6 pt-12 pb-6 min-h-75">
        <div className="grid grid-cols-3 gap-3 mb-8 w-full">
          <div className="bg-zinc-900/50 border border-zinc-800 p-4 rounded-2xl text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Pronoun</p>
            <p className="text-xl font-black text-indigo-400">{currentConjugation.pronoun}</p>
          </div>
          <div className="bg-zinc-900/50 border border-zinc-800 p-4 rounded-2xl text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Verb</p>
            <p className="text-xl font-black text-emerald-400">{currentConjugation.verbBase}</p>
          </div>
          <div className="bg-zinc-900/50 border border-zinc-800 p-4 rounded-2xl text-center">
            <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Tense</p>
            <p className="text-[11px] font-black text-rose-400 leading-tight flex items-center justify-center h-full pt-2">{currentConjugation.tense}</p>
          </div>
        </div>

        <div className="text-center mb-12">
          <p className="text-zinc-600 text-sm font-bold uppercase tracking-widest mb-4">Conjugate correctly</p>
          <div className={`inline-block px-10 py-6 rounded-3xl border-2 border-dashed transition-all duration-300 ${feedback === 'correct' ? 'bg-emerald-500 border-emerald-400 scale-110 shadow-2xl' : feedback === 'error' ? 'bg-rose-500 border-rose-400 scale-110 shadow-2xl' : 'border-zinc-800 bg-zinc-900/30'}`}>
             <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter italic">
              {feedback === 'correct' ? currentConjugation.answer : feedback === 'error' ? 'Oops!' : '????'}
            </h2>
          </div>
        </div>
      </div>
          ...
          {options.map((option, index) => (
            <button
              ...
              className={`h-20 rounded-2xl border-b-[5px] transition-all active:translate-y-1.5 active:border-b-0
                ${feedback === 'correct' && option === currentConjugation.answer ? 'bg-emerald-500 border-emerald-700 text-white shadow-lg' :
                  feedback === 'error' && option !== currentConjugation.answer ? 'bg-rose-950/50 border-rose-950/50 opacity-40 grayscale' :
