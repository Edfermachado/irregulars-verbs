...
export default function PracticeApp() {
  const { score, currentVerb, currentVerbMode: currentMode, options, feedback, checkAnswer, setGameMode } = useStore();
  ...
  const modeLabels = {
    past: { label: 'Simple Past (V2)', color: 'text-indigo-400', bg: 'bg-indigo-500/10', border: 'border-indigo-500' },
    participle: { label: 'Past Participle (V3)', color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500' },
    present3rd: { label: 'Present (3rd Person)', color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500' },
    continuous: { label: 'Present Continuous', color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500' },
    future: { label: 'Future Simple', color: 'text-cyan-400', bg: 'bg-cyan-500/10', border: 'border-cyan-500' },
  };

  if (!mounted || !currentVerb || !currentMode) return null;
        ...
        <div className={`flex flex-col items-center justify-center px-6 py-2.5 rounded-2xl border-2 mb-8 transition-all duration-300 transform shadow-xl
          ${feedback === 'correct' ? 'bg-emerald-500 border-emerald-400 scale-110 shadow-emerald-500/20' :
            feedback === 'error' ? 'bg-rose-500 border-rose-400 scale-110 shadow-rose-500/20' :
              `${modeLabels[currentMode].bg} ${modeLabels[currentMode].border} shadow-lg shadow-zinc-950/20`}`}>

          <span className={`text-[10px] font-bold uppercase tracking-[0.3em] mb-1 opacity-70
             ${feedback ? 'text-white' : modeLabels[currentMode].color.replace('text-', 'text-opacity-70 text-')}`}>
            {feedback ? 'Status' : 'Target Tense'}
          </span>

          <h2 className={`text-xl md:text-2xl font-black uppercase tracking-widest
            ${feedback ? 'text-white' : modeLabels[currentMode].color}`}>
            {feedback === 'correct' ? '¡Excellent!' : feedback === 'error' ? 'Keep trying!' : modeLabels[currentMode].label}
          </h2>
        </div>
